...
def detect_emotion(image: Image.Image):  # Accept PIL Image object as input
    # Convert PIL image to numpy array (as OpenCV works with numpy arrays)
    img = np.array(image.convert('L'))  # Convert to grayscale (L mode in PIL)

    # Preprocess the image (resize, normalize) based on your model's requirements
    img = cv2.resize(img, (48, 48))  # Example resizing (adjust based on model's input)
    img = img.astype('float32') / 255.0  # Normalize pixel values

    # Convert the image to a tensor (as required by PyTorch)
    img_tensor = torch.tensor(img, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
